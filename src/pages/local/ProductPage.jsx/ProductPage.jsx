...
const ProductPage = () => {
	...
	useEffect(() => {
		...
		const fetchData = async () => {
			try {
			  const { data } = await axios.get(`https://techshop-backend-0cyn.onrender.com/api/local/product/${id}`);
			  setData(data);
	...
	useEffect(() => {
		...
		const fetchData = async () => {
			try {
			  const { data } = await axios.get(`https://techshop-backend-0cyn.onrender.com/api/local/get-comment/${id}`);
